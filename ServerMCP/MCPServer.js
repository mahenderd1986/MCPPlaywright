...
class MCPServer {
    browsers = {}; // Map of sessionId to browser/context/page
    currentSession = null; // Current active session id
    llmClient; // LLM client instance
    browserType; // Browser type (e.g., 'chrome', 'firefox')
    port; // Server port
    supportedOperations = {}; // Supported operations: { [operation: string]: string[] }
            ...
            if (req.url === '/execute' && req.method === 'POST') {
                ...
                req.on('end', async () => {
                    try {
                        ...
                        const browser = await this.initializeBrowser(this.browserType, 'headless');
    ...
    static main() {
        ...
        //const server = new McpServer('config.json', 'operations.json');
